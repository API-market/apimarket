...
module.exports = function log(message, data, error, traceOnly = false) {
    //only log this when tracing enabled
    if(traceOnly == true && !(TRACING_ENABLED == "true")) {return;}  
    const environment = (isNullOrEmpty(ENVIRONMENT)) ? 'notspecified' : ENVIRONMENT.toLowerCase();
    //handle error object
  ...
  module.exports = function logTrace(message, data) {
    log(message, data, null, true);
  }
  
  module.exports = function logError(message, error) {
    log(message, null, error, false);
  }
  
  // Log error and throw an error with provided message
  module.exports = logAndThrowError = (message, error) => {
