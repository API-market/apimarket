...
exports.init = async (config, keys, _options = {}) => {
  ...
  if (keys) {
    ...
    let unlocked = await web3.personal.unlockAccount(key, password); // NOTE any new account that gets imported needs to be funded
...
exports.purchase = async offerAddress => {
  const offer = await instrument.at(offerAddress, {
    web3,
    ipfs
  });
...
const open = async voucherAddress => {
  const voucher = await instrument.at(voucherAddress, {
    web3,
    ipfs
  });
  const [apiCapability] = await instrument.exercise(voucher, {
    transactionParameters,
    web3,
