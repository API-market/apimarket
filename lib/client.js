...
exports.init = async (config, keys, _options={}) => {
  ...
  web3.personal.unlockAccount(config.address.holder, config.address.passphrase)
  let address
  if (keys) {
  ...
  return {
    fetch: async (endpoint, request) => {
      const apiCapability = await open(fromRegistry(config, endpoint).voucher.address)
